...
const modDirectories = [
  {
    source: "./BP",
    destination: "development_behavior_packs/manhunt",
  },
  {
    source: "./RP",
    destination: "development_resource_packs/manhunt",
  },
];
...
async function cleanInstallAssets() {
  for (const modDir of modDirectories) {
    ...
    try {
      ...
      console.log(
        `Assets from ${modDir.source} copied to ${modDir.destination}`
      );
    } catch (error) {
